...
public class EntityUtils {
    ...
    public static Field getFieldByAnnotation(Class clazz, Class annotationClass) throws RuntimeException {
        Field field = null;
        Boolean found = false;
        Field[] fields= clazz.getDeclaredFields();
        for(Field declaredField:fields){
            if(declaredField.getAnnotation(annotationClass) != null && !found){
                found = true;
                field = declaredField;
            }else if(found){
                throw new RuntimeException("Multiple annotation of this type");
            }
        }
        if(field != null){
            return field;
