...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(in -> in == name).collect(toList());
    ...
    public static List<Integer> filterNull(){
        ...
        evenNumbers = input.filter(in -> in != null).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personList.stream().filter(person -> Objects.equals(person.getName(), name) && person.getAge() == age).collect(toList());
    ...
    public static int findTheMinimumNumber(){
        ...
        // HINT: use reduce
        int min =0;
        input.reduce(Integer.MAX_VALUE, (acc, x) -> Integer.min(acc,x));
        return min;
    }
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        min = personList.stream().map(Person::getAge).reduce(Integer.MAX_VALUE, Integer::min);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = personList.stream().filter(person -> person.getName().startsWith("J")).map(Person::getNationality).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        firstSquareDivisibleByThree = someNumbers.stream().filter(integer -> (Math.sqrt(integer) == (int) Math.sqrt(integer)) && (integer % 3) == 0).findFirst();
